...
├── 测试/                    # 综合测试 + SQLite / 持续情绪专项回归
├── 晚晚/                    # Python 业务代码（按功能分中文目录）
│   ├── 机器人/qq_bot.py     #   核心：QQ 消息主循环
│   ├── 人设/                #   人设 / 可衰减持续情绪 / 活人感 / 情绪边界
│   ├── 对话/                #   对话决策层 / 情景记忆与自然遗忘 / 会话上下文
│   ├── 成长/                #   日记 / 性格演化 / 亲密度阶段 / 称号
│   ├── 客户端/              #   DeepSeek / OpenAI 兼容 LLM 客户端
│   ├── 配置/                #   config.py + llm_providers.py（.env.example 模板）
│   ├── 图片/ 语音/ 空间/    #   生图 / TTS / ASR / QQ 空间
│   ├── 互动/ 实时/ 用量/    #   自主互动 / 连续生活状态 / 天气搜索 / Token 统计
...
模块分派可为对话、文字任务和看图分别获取模型列表；从下拉列表选择会立即保存，手动输入模型名后点击“应用”保存。
人设 / 参数在 GUI 保存后写入 `晚晚/配置/.runtime_config.json`。
「陪伴状态」是独立一级页面，集中展示实时状态、关系进度、生活轨迹与性格成长。
「素材库」支持预览生成图片、查看人设图片和播放语音缓存；仅生成图片与语音缓存可在二次确认后删除，人设图片始终只读。
...
覆盖：
1. **模块导入** —— 32 个业务模块全部可导入
2. **配置加载** —— config / runtime 正常（无 .env 时用默认值，不报错）
3. **成长系统** —— 性格阶段 / 连续生活线 / 持续情绪 / 对话策略 / 情景记忆正常
4. **提供商配置** —— llm_providers.json 读写正常
5. **GUI 冒烟** —— PySide6 构建 7 个顶级页，校验陪伴状态分组和素材删除权限（窗口 1.5 秒自动关闭）
